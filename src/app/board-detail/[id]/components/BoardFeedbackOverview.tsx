...
const BoardFeedbackOverview: React.FC<BoardFeedbackOverviewProps> = ({ board, feedbackData, feedbackStats, reviewerCount }) => {
  const [feedbackStatsState, setFeedbackStats] = useState<FeedbackStats>({
    totalReactions: 0,
    positiveReactions: 0,
    negativeReactions: 0,
    neutralReactions: 0,
  ...
  useEffect(() => {
    ...
    // Si tenemos datos de feedback desde useBoardDetail, usarlos
    if (feedbackStats && reviewerCount !== undefined) {
      setActualReviewerCount(reviewerCount);
      setFeedbackStats({
        totalReactions: feedbackStats.totalReactions,
        positiveReactions: feedbackStats.positiveReactions,
        negativeReactions: feedbackStats.negativeReactions,
        neutralReactions: feedbackStats.neutralReactions,
  ...
  }, [board?.id, feedbackData, feedbackStats, reviewerCount, board?.reviewCount]); 
  
  return (
    <div className="mt-8 mb-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-light text-gray-700">Resumen de Feedback</h2>
        <div className="text-sm bg-gray-100 rounded-full px-3 py-1 text-gray-700">
          ...
          {feedbackData ? feedbackData.reviewers.length : (reviewerCount !== undefined ? reviewerCount : (board?.reviewCount || 0))} revisores
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border-b border-gray-100 py-3">
          <div className="flex items-center mb-2">
            <div className="w-8 h-8 flex items-center justify-center">
              <span className="text-2xl font-light text-gray-700">{feedbackStatsState.totalReactions}</span>
            </div>
          ...
          <div className="flex items-center pl-11">
            <div className="flex-1">
              <div className="h-1.5 bg-gray-100 rounded-full w-full overflow-hidden flex">
                {feedbackStatsState.totalReactions > 0 ? (
                  <>
                    <div 
                      className="h-full bg-gray-600" 
                      style={{ width: `${(feedbackStatsState.positiveReactions / feedbackStatsState.totalReactions) * 100}%` }}
                    ></div>
                    <div 
                      className="h-full bg-gray-300" 
                      style={{ width: `${(feedbackStatsState.negativeReactions / feedbackStatsState.totalReactions) * 100}%` }}
                    ></div>
                  </>
                ) : null}
              </div>
              <div className="flex text-xs text-gray-500 mt-2 justify-between">
                <div className="flex items-center">
                  ...
                  <span>{feedbackStatsState.positiveReactions}</span>
                </div>
                <div className="flex items-center">
                  <ThumbsDown className="w-3 h-3 text-gray-500 mr-1" />
                  <span>{feedbackStatsState.negativeReactions}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white border-b border-gray-100 py-3">
          <div className="flex items-center mb-2">
            <div className="w-8 h-8 flex items-center justify-center">
              <span className="text-2xl font-light text-gray-700">{feedbackStatsState.totalComments}</span>
            </div>
          ...
          <div className="flex items-center pl-11">
            <div className="space-y-1 flex-1">
              {feedbackStatsState.loading ? (
                <p className="text-xs text-gray-400">Loading...</p>
              ) : feedbackStatsState.recentComments.length > 0 ? (
                feedbackStatsState.recentComments.map((comment, index) => (
